...
export default class Application extends React.Component {
  ...
  handleSearch(search) {
    for (let i = 0; i < this.state.messagesArray.length; i++) {
      if (this.state.messagesArray[i].input === search) {
        searchResults.push(this.state.messagesArray[i]);
        //new this.state.searchResults
        //push to searchResults
      }
    }
  }
    ...
    return (
      <div className='application'>
        <FilterBar
          ...
          />
        <RenderMessages
          messages={this.state.searchResults || this.state.messagesArray}
          user={this.state.searchResults.displayName || this.state.user}
          />
        <Login
          authorize={signIn}
